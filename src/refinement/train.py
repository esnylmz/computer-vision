...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        # Optional extra fields (used by constraint-aware loss)
        if 'pitches' in item:
            pitches = torch.cat([
                item['pitches'],
                torch.zeros(pad_len, dtype=torch.long)
            ...
            pitches = torch.zeros(max_len, dtype=torch.long)
        ...
        if 'hands' in item:
            ...
            hands = torch.zeros(max_len, dtype=torch.long)
    ...
    return {
        ...
        'pitches': torch.stack(pitches_list),
        'hands': torch.stack(hands_list),
    }
